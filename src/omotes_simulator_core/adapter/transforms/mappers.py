...
from esdl.esdl import Pipe as esdl_Pipe
...
from omotes_simulator_core.adapter.transforms.esdl_asset_mapper import EsdlAssetPipeMapper
    ...
    def _convert_assets(self, network: Network) -> List[AssetAbstract]:
        ...
        for esdl_asset in self.esdl_object.get_all_assets_of_type("asset"):
            ...
            if isinstance(esdl_asset.esdl_asset, esdl_junction):
                continue
            if isinstance(esdl_asset.esdl_asset, esdl_Pipe):
                py_asset = EsdlAssetPipeMapper().to_entity(esdl_asset=esdl_asset)
            else:
